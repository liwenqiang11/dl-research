...
2. full-auto：全自动推进，允许绕过用户确认门控，但不能绕过事实核验、形式推导核验、独立证据审核、行动合规等认识论/安全门控。
...
Default mode is **guided** only after the user confirms activation/continuation but does not choose a mode. In **full-auto** mode, user-confirmation gates may be bypassed, but epistemic and safety gates cannot be bypassed. Atomic Fact Verification, Formal Derivation Verification, Independent Evidence Auditor Gate, Pre-Action Compliance, Branch Plan, and Run-to-Branch Binding still run whenever triggered. The agent must record which user-confirmation gates were bypassed, why bypassing was chosen, and what risk remains. This only changes dl-research workflow behavior; it does not override higher-priority system, tool, security, or user permission requirements.
